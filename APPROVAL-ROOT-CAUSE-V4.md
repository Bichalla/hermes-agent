...
> **V5 조사로 정정됨:** 이 보고서의 SSH 거절을 정상 범위 제한으로 단정한 부분은 불완전한 입력에 근거했다. PM 댓글 346은 이미 제한된 Oracle 읽기 전용 조회를 허용했지만 V4 broker가 이 인계를 전달하지 않았다. 또한 연결은 work-executor에만 설치돼 run561 리뷰어에게 적용되지 않았다. V4의 `ready=true`와 테스트 성공은 전체 역할의 정상 동작을 입증하지 못했다. 현재 판정과 정확한 run560 명령의 추가 범위 위반은 [V5 보고서](APPROVAL-ROOT-CAUSE-V5.md)를 따른다. 아래는 당시 관측 이력이다.
