...
def _response_get(value: Any, key: str, default: Any = None) -> Any:
    """Read a field from either an SDK object or a raw response dict."""
    if isinstance(value, dict):
        return value.get(key, default)
    return getattr(value, key, default)


def _response_set(value: Any, key: str, field_value: Any) -> None:
    """Best-effort field assignment for SDK objects and raw response dicts."""
    if isinstance(value, dict):
...
def _extract_responses_message_text(item: Any) -> str:
    ...
    content = _response_get(item, "content")
    if not isinstance(content, list):
        return ""

    chunks: List[str] = []
    for part in content:
        ptype = _response_get(part, "type")
        if ptype not in {"output_text", "text"}:
            continue
        text = _response_get(part, "text")
        if isinstance(text, str) and text:
            chunks.append(text)
...
def _extract_responses_reasoning_text(item: Any) -> str:
    ...
    summary = _response_get(item, "summary")
    if isinstance(summary, list):
        chunks: List[str] = []
        for part in summary:
            text = _response_get(part, "text")
...
def _normalize_codex_response(response: Any) -> tuple[Any, str]:
    ...
    if not isinstance(output, list) or not output:
        ...
        out_text = _response_get(response, "output_text")
        if isinstance(out_text, str) and out_text.strip():
            ...
            _response_set(response, "output", output)
        else:
            raise RuntimeError("Responses API returned no output items")

    response_status = _response_get(response, "status")
    ...
    if response_status in {"failed", "cancelled"}:
        error_obj = _response_get(response, "error")
    ...
    for item in output:
        item_type = _response_get(item, "type")
        item_status = _response_get(item, "status")
        ...
        if item_type == "message":
            item_phase = _response_get(item, "phase")
            ...
            encrypted = _response_get(item, "encrypted_content")
            if isinstance(encrypted, str) and encrypted:
            ...
            if item_status in {"queued", "in_progress", "incomplete"}:
                continue
            fn_name = _response_get(item, "name", "") or ""
            arguments = _response_get(item, "arguments", "{}")
            if not isinstance(arguments, str):
                arguments = json.dumps(arguments, ensure_ascii=False)
            raw_call_id = _response_get(item, "call_id")
            raw_item_id = _response_get(item, "id")
            ...
            fn_name = _response_get(item, "name", "") or ""
            arguments = _response_get(item, "input", "{}")
            if not isinstance(arguments, str):
                arguments = json.dumps(arguments, ensure_ascii=False)
            raw_call_id = _response_get(item, "call_id")
            raw_item_id = _response_get(item, "id")
