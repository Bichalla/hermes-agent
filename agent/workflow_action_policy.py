...
_ADAPTER_IDS = frozenset(
    {
        ...
        "company-work-os-operating-recorder",
        "company-work-os-team-roster-recorder",
        "kanban-status-memory",
        "kanban-intake-pending",
        "lifelog-childcare-recorder",
        "lifelog-diet-recorder",
        "lifelog-family-event-recorder",
        "lifelog-medication-recorder",
        "lifelog-semantic-debug-issuer",
        "lifelog-sleep-recorder",
...
_REGISTERED_CAPABILITIES: Mapping[str, RegisteredCapability] = MappingProxyType(
    {
        "company-work-os.team-roster-seed.v1": RegisteredCapability(
            capability_id="company-work-os.team-roster-seed.v1",
            effects=frozenset({WorkflowEffect.CREATE}),
            authority_modes=frozenset({AuthorityMode.FOREGROUND_CURRENT_TURN}),
            adapter_id="company-work-os-team-roster-recorder",
            input_schema_id="company-work-os-team-roster-seed/v1",
            result_schema_id="company-work-os-team-roster-seed-result/v1",
            idempotency=IdempotencyMode.DETERMINISTIC_REPLAY,
            readback_required=True,
            soft_delete_restore_required=False,
        ),
        "company-work-os.operating-record.v1": RegisteredCapability(
            capability_id="company-work-os.operating-record.v1",
            effects=frozenset({WorkflowEffect.CREATE}),
            authority_modes=frozenset({AuthorityMode.FOREGROUND_CURRENT_TURN}),
            adapter_id="company-work-os-operating-recorder",
        ...
        "lifelog.family-event.v1": RegisteredCapability(
            capability_id="lifelog.family-event.v1",
            effects=frozenset({WorkflowEffect.CREATE, WorkflowEffect.UPDATE}),
            authority_modes=frozenset({AuthorityMode.FOREGROUND_CURRENT_TURN}),
            adapter_id="lifelog-family-event-recorder",
            input_schema_id="family-event-bundle/v1",
            result_schema_id="registered-local-workflow-result/v1",
            idempotency=IdempotencyMode.DETERMINISTIC_REPLAY,
            readback_required=True,
            soft_delete_restore_required=False,
        ),
        "lifelog.medication-intake.v1": RegisteredCapability(
            capability_id="lifelog.medication-intake.v1",
            effects=frozenset({WorkflowEffect.CREATE}),
            authority_modes=frozenset({AuthorityMode.FOREGROUND_CURRENT_TURN}),
            adapter_id="lifelog-medication-recorder",
        ...
        "lifelog.sleep-record.v1": RegisteredCapability(
            capability_id="lifelog.sleep-record.v1",
            effects=frozenset({WorkflowEffect.CREATE}),
            authority_modes=frozenset({AuthorityMode.FOREGROUND_CURRENT_TURN}),
            adapter_id="lifelog-sleep-recorder",
            input_schema_id="lifelog-sleep-record/v1",
            result_schema_id="registered-recorder-result/v1",
            idempotency=IdempotencyMode.DETERMINISTIC_REPLAY,
            readback_required=True,
            soft_delete_restore_required=False,
...
_REGISTERED_OPERATIONS: Mapping[tuple[str, str], WorkflowEffect] = MappingProxyType(
    {
        (
            "company-work-os.team-roster-seed.v1",
        ...
        ("lifelog.family-event.v1", "family_event_record"): WorkflowEffect.CREATE,
        ("lifelog.family-event.v1", "family_event_correct"): WorkflowEffect.UPDATE,
        ("lifelog.medication-intake.v1", "medication_intake_record"): WorkflowEffect.CREATE,
        ...
        ("lifelog.sleep-record.v1", "sleep_record"): WorkflowEffect.CREATE,
...
_REGISTERED_OPERATION_INPUT_SCHEMAS: Mapping[tuple[str, str], str] = MappingProxyType(
    {
        key: _REGISTERED_CAPABILITIES[key[0]].input_schema_id
        for key in _REGISTERED_OPERATIONS
    }
    | {
        ("lifelog.family-event.v1", "family_event_record"): "family-event-bundle/v1",
        ("lifelog.family-event.v1", "family_event_correct"): "family-event-correction/v1",
    }
)
_REGISTERED_OPERATION_AUTHORITIES: Mapping[
...
] = MappingProxyType(
    {
        ...
        ("lifelog.family-event.v1", "family_event_record"): frozenset(
            {AuthorityMode.FOREGROUND_CURRENT_TURN}
        ),
        ("lifelog.family-event.v1", "family_event_correct"): frozenset(
            {AuthorityMode.FOREGROUND_CURRENT_TURN}
        ),
        ("lifelog.medication-intake.v1", "medication_intake_record"): frozenset(
            {AuthorityMode.FOREGROUND_CURRENT_TURN}
        ),
        ...
        ("lifelog.sleep-record.v1", "sleep_record"): frozenset(
            {AuthorityMode.FOREGROUND_CURRENT_TURN}
        ),
