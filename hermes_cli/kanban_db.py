    ...
    *,
    ...
    with write_txn(conn):
        ...
        if current is None or current.status != "ready":
            return None
        # Change Gate is authoritative for every implementation claim.
        _check_change_gate_before_claim(conn, task_id, board=board)
        # Repository availability is an additional constraint, never an
        # alternate authorization path. Re-read it inside the claim transaction.
        if _repo_aware:
...
def dispatch_once(
    ...
    *,
    ...
    """
    # Dry runs are read-only queue projections and must never create the
    # durable dispatch lock. Mode-off retains the historical dispatch body
    # below, but the outer lock is still bypassed for dry-run purity.
    if dry_run:
