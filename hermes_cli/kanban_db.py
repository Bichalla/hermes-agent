...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    """Return True when ``task_id`` has an unreleased sticky block.
    ...
    * **Initial operator parking** — ``create_task(initial_status="blocked")``
      records ``status="blocked"`` in the creation event.  This is also a
      deliberate hold and must survive until a manual promote or unblock.
    ...
    * **Circuit-breaker** — ``_record_task_failure`` tripped after
    ...
    block-lifecycle event for the task.  If the most recent one is
    ``"blocked"`` (or the task was initially created with
    ``status="blocked"`` and no ``"unblocked"``/``"promoted_manual"``
    event has fired since), the task is sticky and ``recompute_ready``
    must *not* auto-promote it.
    ...
    if row["kind"] == "blocked":
        return True
    if row["kind"] in {"unblocked", "promoted_manual"}:
        return False
    try:
