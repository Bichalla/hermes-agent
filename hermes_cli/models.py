...
def detect_static_provider_for_model(
    ...
    # In auto mode, prefer the Codex OAuth runtime for models that Hermes
    # explicitly curates for Codex. Some of these IDs also appear in the
    # generic OpenAI API catalog, whose insertion order would otherwise route
    # them to openai-api before openai-codex.
    if "auto" in current_keys:
        codex_models = _PROVIDER_MODELS.get("openai-codex", [])
        if any(name_lower == m.lower() for m in codex_models):
            return ("openai-codex", name)
