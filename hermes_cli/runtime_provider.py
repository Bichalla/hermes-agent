...
def resolve_runtime_provider(
    *,
    ...
    # resolve_provider() pick up a cloud/provider API key from the environment.
    # This must run before model-owner auto-detection so a local/custom
    # OpenAI-compatible endpoint can intentionally serve models with cloud-like names.
    if requested_provider == "auto" and not explicit_base_url and not explicit_api_key:
        cfg_provider = str(model_cfg.get("provider") or "").strip().lower()
