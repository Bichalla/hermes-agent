...
import pytest
import agent.medication_intake_intent as medication_intent_module
...
def test_hostile_catalog_baseexception_is_constant_and_empty() -> None:
    marker = "PRIVATE-CATALOG-BASE"

    class HostileStop(BaseException):
        pass

    class HostileDict(dict[str, frozenset[str]]):
        def items(self):
            raise HostileStop(marker)

    with pytest.raises(ValueError, match="^catalog_invalid$") as exc_info:
        MedicationCatalog(
            alias_to_keys=MappingProxyType(HostileDict()),
            retired_keys=frozenset(),
            catalog_nonce=NONCE,
...
def test_catalog_snapshot_does_not_swallow_process_or_memory_signals(
...
def test_hostile_monotonic_baseexception_returns_deadline_exceeded(
    fail_on_call: int,
) -> None:
    marker = "PRIVATE-MONOTONIC-BASE"
...
def test_projection_replaces_overlapping_name_alias_ingredient_and_dose_spans_atomically() -> None:
    from tools.medication_intake_config import load_medication_intake_catalog

    def load(entries: list[dict[str, object]]):
        return load_medication_intake_catalog(
            {
                "registered_workflow": {
                    "medication_intake": {
                        "enabled": True,
                        ...
                        "allowed_thread_ids": ["synthetic-thread"],
                        "catalog_nonce": NONCE,
                        "retired_keys": [],
                        "entries": entries,
                    }
    ...
    entries = [
        {
            ...
            "aliases": ["alpha tablet"],
            "ingredient_markers": ["alpha"],
            "dose_markers": ["synthetic alpha 10mg"],
        },
        {
            "medication_key": KEY_B,
            "canonical_name": "synthetic beta",
            "aliases": [],
            "ingredient_markers": ["beta ingredient"],
            "dose_markers": [],
        },
    ]
    catalog = load(entries)
    reversed_catalog = load(list(reversed(json.loads(json.dumps(entries)))))
    projector = getattr(medication_intent_module, "project_medication_catalog_text")
    text = "synthetic alpha 10mg and alpha tablet, then synthetic alpha"
    projected = projector(text, catalog, stable_session_identity="stable-session")
    restarted = projector(
        text,
        reversed_catalog,
        stable_session_identity="stable-session",
    )

    assert projected.denial_code is None
    assert projected.projected_text == restarted.projected_text
    assert len(projected.ref_to_key) == 1
    projection_ref = next(iter(projected.ref_to_key))
    assert projected.projected_text == (
        f"{projection_ref} and {projection_ref}, then {projection_ref}"
    )
    for private_span in (
        "synthetic alpha",
        "alpha tablet",
        ...
        assert private_span not in projected.projected_text

    partial_overlap = load(
        [
            {
                "medication_key": KEY_A,
                "canonical_name": "first synthetic",
                "aliases": [],
                "ingredient_markers": ["overlap marker"],
                "dose_markers": [],
            },
            {
                "medication_key": KEY_B,
                ...
                "aliases": [],
                "ingredient_markers": ["marker tail"],
                "dose_markers": [],
            },
        ]
    )
    ...
    assert denied.denial_code == "projection_overlap"
    assert denied.projected_text == "[private medication context unavailable]"
    assert denied.ref_to_key == {}

    overflow = projector(
        ("synthetic alpha " * (MAX_INPUT_CHARS // len("synthetic alpha ") + 1)),
        catalog,
        stable_session_identity="stable-session",
    )
    assert overflow.denial_code == "projection_input_overflow"

    malformed_catalog = load(json.loads(json.dumps(entries)))
    object.__setattr__(malformed_catalog, "catalog_digest", "0" * 64)
    malformed = projector(
        text,
        malformed_catalog,
        stable_session_identity="stable-session",
    )
    assert malformed.denial_code == "projection_catalog_invalid"

    retired_catalog = MedicationCatalog(
        alias_to_keys={"retired synthetic": frozenset({KEY_A})},
        retired_keys=frozenset({KEY_A}),
        catalog_nonce=NONCE,
        catalog_digest=CATALOG_DIGEST,
    )
    medication_intent_module._seal_validated_medication_catalog(
        retired_catalog,
        CATALOG_DIGEST,
    )
    retired = projector(
    ...
    assert retired.denial_code == "projection_retired"

    path = Path(__file__).parents[2] / "agent" / "medication_intake_intent.py"
    tree = ast.parse(path.read_text(encoding="utf-8"))
    projection_function = next(
        node
        for node in tree.body
        if isinstance(node, ast.FunctionDef)
        and node.name == "project_medication_catalog_text"
    )
    called = {
        node.func.id
...
def test_public_callback_boundaries_normalize_but_control_flow_propagates(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    marker = "PRIVATE-PUBLIC-CALLBACK"

    class HostileFailure(BaseException):
        pass

    cancellation_error = type("CancellationError", (RuntimeError,), {})
    normalized_types = (RuntimeError, HostileFailure)
    propagated_types = (
        KeyboardInterrupt,
        SystemExit,
        GeneratorExit,
    ...
    def raised(exception: BaseException):
    ...
    for boundary in (
        ...
        "binding",
        "projector_normalize",
        "projector_marker_iter",
        "projector_ref",
        "initial_clock",
        "later_clock",
    ):
        for exception_type in (*normalized_types, *propagated_types):
            exception = exception_type(marker)
            with monkeypatch.context() as scoped:
                operation: Callable[[], Any]
                if boundary == "seal":
                    scoped.setattr(medication_intent_module.json, "dumps", raised(exception))
                    operation = lambda: medication_intent_module._seal_validated_medication_catalog(
                        _catalog(), CATALOG_DIGEST
                    )
                    ...
                    scoped.setattr(medication_intent_module.json, "dumps", raised(exception))
                    operation = lambda: medication_intent_module.project_medication_catalog_text(
                        "합성알파 먹었어",
                        catalog,
                        stable_session_identity="stable-session",
                    )
                elif boundary.startswith("projector_"):
                    catalog = _catalog()
                    medication_intent_module._seal_validated_medication_catalog(
                        catalog, CATALOG_DIGEST
                    )
                    if boundary == "projector_normalize":
                        scoped.setattr(
                            medication_intent_module,
                            "_normalize_text",
                            raised(exception),
                        )
                    elif boundary == "projector_marker_iter":
                        class HostileSpans:
                            def __iter__(self):
                                raise exception
                    ...
                    def clock() -> float:
                        nonlocal calls
                        calls += 1
                        if boundary == "initial_clock" or calls > 1:
                            raise exception
                        return 10.0

                    operation = lambda: classify_medication_intake(
                        "합성알파 먹었어",
                        _catalog(),
                        monotonic=clock,
                    )

                if exception_type in normalized_types:
                    if boundary == "seal":
                        with pytest.raises(ValueError, match="^catalog_invalid$") as exc_info:
                            operation()
                        _assert_private_absent(marker, exc_info.value, repr(exc_info.value))
                        ...
                        result = operation()
                        _assert_private_absent(marker, result, repr(result))
                        if boundary == "binding" or boundary.startswith("projector_"):
                            assert result.projected_text == "[private medication context unavailable]"
                            assert result.ref_to_key == {}
...
def test_projector_full_guard_does_not_hide_local_result_construction_errors(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    catalog = _catalog()
    medication_intent_module._seal_validated_medication_catalog(
    ...
    class LocalImplementationError(AssertionError):
        pass

    def broken_result(**_kwargs: object) -> object:
        raise LocalImplementationError("local-construction-bug")
    ...
    with pytest.raises(LocalImplementationError, match="^local-construction-bug$"):
        medication_intent_module.project_medication_catalog_text(
            "ordinary text",
            catalog,
            stable_session_identity="stable-session",
        )
