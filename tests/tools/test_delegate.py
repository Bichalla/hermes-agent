...
class TestDelegateHeartbeat(unittest.TestCase):
    ...
    def test_heartbeat_does_not_trip_idle_stale_while_inside_tool(self):
        ...
        def slow_run(**kwargs):
            # Wait until the second heartbeat has a chance to fire. With the
            # old bug and an idle stale limit of 1, the heartbeat loop stops
            # before touching the parent a second time while current_tool is
            # still set.
        ...
        # Force a sharp distinction between idle and in-tool stale limits:
        # old behavior (using the idle limit while a tool is running) would
        # stop after one stale cycle; fixed behavior keeps heartbeating.
        with patch("tools.delegate_tool._HEARTBEAT_INTERVAL", 0.05), \
