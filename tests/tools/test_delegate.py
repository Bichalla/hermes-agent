...
class TestDelegateHeartbeat(unittest.TestCase):
    ...
    def test_heartbeat_does_not_trip_idle_stale_while_inside_tool(self):
        ...
        def slow_run(**kwargs):
            # Wait until the third heartbeat has a chance to fire. With the
            # old bug and a tiny idle stale limit, the heartbeat loop stops
            # before touching the parent a third time while current_tool is
            # still set.
            deadline = time.monotonic() + 1.5
            while len(touch_calls) < 3 and time.monotonic() < deadline:
                time.sleep(0.01)
