...
def test_loads_immutable_alias_to_random_key_catalog_and_nonce_salted_digest() -> None:
    ...
    expected = hmac.new(
        bytes.fromhex(nonce),
        b"lifelog-medication-catalog/v4\0" + payload,
        hashlib.sha256,
    ).hexdigest()
...
def test_catalog_digest_uses_exact_validated_config_excluding_only_nonce() -> None:
    ...
    expected = hmac.new(
        bytes.fromhex(nonce),
        b"lifelog-medication-catalog/v4\0" + expected_payload,
        hashlib.sha256,
    ).hexdigest()
    catalog = _load(config)
    assert catalog.catalog_digest == expected
...
def test_state_visible_digest_resists_name_dictionary_without_private_nonce() -> None:
    ...
    dictionary_candidates = (
        b"lifelog-medication-catalog/v4\0" + public_shape,
        b"lifelog-medication-catalog/v4\0" + "합성알파".encode(),
        b"lifelog-medication-catalog/v4\0" + "알파정".encode(),
    )
...
def test_optional_private_detail_markers_and_restart_stable_projection_material_are_closed() -> None:
    config, nonce, key_a, key_b = _config()
    medication = config["registered_workflow"]["medication_intake"]  # type: ignore[index]
    entries = medication["entries"]  # type: ignore[index]
    entries[0]["ingredient_markers"] = ["합성성분공통", "합성성분알파"]  # type: ignore[index]
    entries[0]["dose_markers"] = ["합성용량알파"]  # type: ignore[index]
    entries[1]["ingredient_markers"] = ["합성성분공통"]  # type: ignore[index]
    entries[1]["dose_markers"] = []  # type: ignore[index]

    catalog = _load(config)
    restarted = _load(config, previous_catalog_nonce=nonce)
    assert catalog.alias_to_keys["합성성분공통"] == frozenset({key_a, key_b})
    assert catalog.alias_to_keys["합성성분알파"] == frozenset({key_a})
    assert catalog.alias_to_keys["합성용량알파"] == frozenset({key_a})
    assert restarted.catalog_digest == catalog.catalog_digest

    projector = getattr(medication_intent_module, "project_medication_catalog_text")
    first = projector("합성성분알파 복용", catalog, stable_session_identity="session-a")
    second = projector("합성성분알파 복용", restarted, stable_session_identity="session-a")
    expected_ref = "med-ref-" + hmac.new(
        bytes.fromhex(nonce),
        b"lifelog-medication-projection-ref/v1\0session-a\0" + bytes.fromhex(key_a),
        hashlib.sha256,
    ).hexdigest()
    assert first.projected_text == second.projected_text
    assert first.projected_text == f"{expected_ref} 복용"
    assert first.ref_to_key == {expected_ref: key_a}
    assert repr(first) == "MedicationCatalogProjection(<opaque>)"
    _assert_private_absent(nonce, first, repr(first), first.projected_text)
    with pytest.raises(Exception, match="^serialization_denied$"):
        pickle.dumps(catalog)
    with pytest.raises(TypeError):
        dataclasses.asdict(catalog)  # type: ignore[arg-type]

    legacy, *_ = _config()
    assert _load(legacy).catalog_digest

    ambiguous = projector(
        "합성성분공통 복용",
        catalog,
        stable_session_identity="session-a",
    )
    assert ambiguous.denial_code == "projection_ambiguous"
    assert ambiguous.projected_text == "[private medication context unavailable]"
    assert ambiguous.ref_to_key == {}

    malformed_detail, *_ = _config()
    malformed_entries = malformed_detail["registered_workflow"]["medication_intake"]["entries"]  # type: ignore[index]
    malformed_entries[0]["ingredient_markers"] = "not-a-list"  # type: ignore[index]
    with pytest.raises(MedicationIntakeConfigError, match="^catalog_detail_invalid$"):
        _load(malformed_detail)

    overbound_detail, *_ = _config()
    overbound_entries = overbound_detail["registered_workflow"]["medication_intake"]["entries"]  # type: ignore[index]
    overbound_entries[0]["dose_markers"] = [f"dose-{index}" for index in range(256)]  # type: ignore[index]
    with pytest.raises(MedicationIntakeConfigError, match="^catalog_detail_limit$"):
        _load(overbound_detail)


def test_config_callbacks_normalize_but_control_flow_propagates(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    marker = "PRIVATE-CONFIG-CALLBACK"

    class HostileFailure(BaseException):
        pass

    cancellation_error = type("CancelledError", (RuntimeError,), {})
    ...
    for boundary in ("normalize", "catalog"):
        for exception_type in (*normalized_types, *propagated_types):
            ...
            with monkeypatch.context() as scoped:
                if boundary == "normalize":
                    ...
                    expected_code = "catalog_alias_invalid"
                else:
                    scoped.setattr(
                        medication_config_module,
