...
from agent.medication_name_contract import normalize_medication_name
...
_ENTRY_REQUIRED_FIELDS = frozenset({"medication_key", "canonical_name", "aliases"})
_ENTRY_OPTIONAL_FIELDS = frozenset({"ingredient_markers", "dose_markers"})
...
_CONFIG_ERROR_CODES = frozenset(
    {
        ...
        "catalog_detail_invalid", "catalog_detail_limit",
    ...
    try:
        ...
        if previous_key_bindings is None:
            ...
            for previous_key, previous_name in previous_key_bindings.items():
                ...
                try:
                    normalized_previous_bindings[previous_key] = normalize_medication_name(
                        previous_name
                    )
        ...
        for entry in entries:
            if (
                ...
                or not _ENTRY_REQUIRED_FIELDS.issubset(entry)
                or not frozenset(entry).issubset(_ENTRY_FIELDS)
            ):
                _fail("catalog_entry_invalid")
            raw_aliases = entry["aliases"]
            if type(raw_aliases) is not list:
                _fail("catalog_alias_invalid")
            marker_count += len(raw_aliases)
            if marker_count > MAX_CATALOG_ALIASES:
            ...
            if any(
                ...
                _fail("catalog_alias_invalid")
            for field in _ENTRY_OPTIONAL_FIELDS:
                raw_markers = entry.get(field, [])
                if type(raw_markers) is not list:
                    _fail("catalog_detail_invalid")
                marker_count += len(raw_markers)
                if marker_count > MAX_CATALOG_ALIASES:
                    _fail("catalog_detail_limit")
                if any(
                    type(marker) is not str
                    or not marker
                    or len(marker) > MAX_ALIAS_CHARS
                    for marker in raw_markers
                ):
                    _fail("catalog_detail_invalid")
        marker_to_keys: dict[str, set[str]] = {}
        name_alias_to_key: dict[str, str] = {}
        ...
        for entry in entries:
            ...
            try:
                canonical_name = normalize_medication_name(entry["canonical_name"])
            except BaseException as exc:
                _raise_if_control_flow(exc)
                _fail("catalog_alias_invalid")
            ...
            for alias in raw_aliases:
                try:
                    normalized_aliases.append(normalize_medication_name(alias))
                except BaseException as exc:
                    _raise_if_control_flow(exc)
                    _fail("catalog_alias_invalid")
            aliases = [canonical_name, *normalized_aliases]
            for alias in aliases:
                if alias in name_alias_to_key:
                    _fail("catalog_alias_duplicate")
                for existing in name_alias_to_key:
                    if _overlaps(alias, existing):
                        _fail("catalog_alias_overlap")
                name_alias_to_key[alias] = medication_key
                marker_to_keys.setdefault(alias, set()).add(medication_key)
            for field in _ENTRY_OPTIONAL_FIELDS:
                for raw_marker in entry.get(field, []):
                    try:
                        marker = normalize_medication_name(raw_marker)
                    except BaseException as exc:
                        _raise_if_control_flow(exc)
                        _fail("catalog_detail_invalid")
                    marker_to_keys.setdefault(marker, set()).add(medication_key)


        if len(marker_to_keys) > MAX_CATALOG_ALIASES:
            _fail("catalog_alias_limit")
        ...
        catalog: MedicationCatalog | None = None
        try:
            catalog = MedicationCatalog(
                alias_to_keys={
                    marker: frozenset(keys) for marker, keys in marker_to_keys.items()
                },
        ...
        if catalog is None:
            _fail("config_invalid")
        try:
            sealed_catalog = _seal_validated_medication_catalog(catalog, digest)
        except BaseException as exc:
            _raise_if_control_flow(exc)
            sealed_catalog = None
        if sealed_catalog is None:
            _fail("config_invalid")
        return sealed_catalog
